...
export default class CryptoMangaChannel extends EPNSChannel {
    ...
    try {
      ...
      for (const proposal of res.proposals) {
        try {
          ...
          const payloadMsg =
            "A Proposal has been created on cryptoManga [b: " +
            proposal.title +
            "]" +
            "[timestamp:" +
            Date.now() / 1000 +
            "]";
          const message =
            "A Proposal " + proposal.title + "has been created on cryptoManga";
          const title = "New Proposal";
          const cta =
            "https://snapshot.org/#/mangabank.eth/proposal/" + proposal.id;
      ...
      await this.setcryptoMangaDataInDB({
    ...
    const snapshotQuery = gql`
      {
        proposals(
          orderBy: "start"
          orderDirection: desc
          where: { space_in: ["mangabank.eth"], created_gte: createdGte }
        ) {
          id
          title
          body
        ...
        try {
          if (maxPercentage && resultChoice) {
            ...
            const message =
              'A Proposal "' +
              proposal.title +
              "has been concluded on cryptoManga";
    ...
    for (const proposal of res.proposals) {
      ...
      if (diff <= 86400 && diff > 0) {
        try {
          ...
          const payloadMsg =
            "A Proposal is concluding soon on cryptoManga[b:" +
            proposal.title +
            "]";
          const message =
            'A Proposal "' +
            proposal.title +
            '" is concluding soon on cryptoManga ' +
            proposal.end;
  ...
  async setcryptoMangaDataInDB(cryptoMangaData: IcryptoMangaData) {
    ...
    await cryptoMangaModel.findOneAndUpdate(
      { _id: "cryptoManga_DATA" },
      cryptoMangaData,
      { upsert: true }
    );
  }
